...
// DFS tries to move through the maze from the current cell and reach the exit
// I also record the parent of every cell so I can rebuild the path afterward
         ...
         int exit_r, int exit_c)
{
    ...
    // If I'm outside the maze then I can't explore this cell
    if (r < 0 || r >= N || c < 0 || c >= M)
        return false;

    // Can't move through walls (1 = wall)
    if (maze[r][c] == 1)
        return false;

    // Skip cells I've already visited
    if (visited[r][c])
        return false;

    // Mark this cell as visited so I do not revisit it
    visited[r][c] = true;

    // If I'm at the exit then DFS succeeded
    if (r == exit_r && c == exit_c)
        return true;

    // Explore 4 directions using dr/dc arrays
    for (int k = 0; k < 4; k++) {
        int nr = r + dr[k];
        int nc = c + dc[k];
        ...
        // Only explore open and unvisited neighbors
        if (maze[nr][nc] == 0 && !visited[nr][nc]) {

            // Set parent of neighbor BEFORE exploring it
            parent_r[nr][nc] = r;
            parent_c[nr][nc] = c;
    ...
    // None of the directions worked meaning this is a dead end
    return false;
}
...
int main() {
    ...
    // ------------------------------------------------------
    // STUDENT WORK:
    // Call your DFS, track visited, and fill parent arrays
    // ------------------------------------------------------
    // Start DFS at the entrance so it explores the maze and fills the visited and parent arrays for me
    ...
    // If DFS found a path I show it from start to exit
    // If not then I say there is no path in this maze
    if (found) {
